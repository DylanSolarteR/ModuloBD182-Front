        ...
        <div className="space-y-4">
            {data.map((item) => (
                <div key={item.consecReque} className="bg-secondaryAccent/50 text-primaryBg p-4 rounded-lg flex items-center justify-between">
                        ...
                        <div className="flex space-x-2">
                            {[...Array(7)].map((_, index) => (
                                <Link href={`/edit-proc-req/${item.consecReque}/${item.ConsProceso}/${index + 1}`} key={index}>
                                    <span className={`px-2 py-1 rounded ${item.idFase === index ? 'bg-primaryAccent text-primaryBg' : 'bg-secondaryBg text-secondaryText'}`}>
                                        {index + 1}
                                    </span>
                                </Link>
                            ))}
                    ...
                    <Link className="px-8" href={`/edit-proc-req/${item.consecReque}/${item.ConsProceso}/${item.idFase + 1}`}>
